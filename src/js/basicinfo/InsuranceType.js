...
var data_col = [
    [{
        ...
        },
        {
            title: '操作',
            templet: function(d) {
                return getBtnHtml(d.isSystem, d.workinStatus);
            },
            width: 180
        }
    ]
...
layui.use(['table', 'element', 'laydate', 'form'], function() {
    ...
    var search = function() {
        return {
            "search": $("input[name='Type_Chinese']").attr('e-value')
        };
    };
    //操作栏的回调函数
            ...
            if (layEvent === 'edit') {
                ...
                if (data.workinStatus == 1) {
                    ...
                    layer_confirm('确定启用信息吗？', function() {
                        layer_load();
                        Serv.Post('gc/InsuranceType/Delete', data, function(result) {
                            if (result.code == "00") {
    ...
    lstPager = Pager2(table, //lay-ui的table控件
        ...
        'upBar', //绑定的工具条Id
        data_col, //表头的显示行
        "gc/InsuranceType/Get", //action url 只能post提交
        search,
    ...
    layform.on('submit(formDemo)', function(laydata) {
        ...
        if (laydata.field.Id == "") {
            ...
            Serv.Post('gc/InsuranceType/add', {
                workerInField: laydata.field
            }, function(response) {
                if (response.code == "00") {
            ...
            if (laydata.field.isSystem == 1) {
                ...
                Serv.Post('gc/InsuranceType/update', laydata.field, function(response) {
                    if (response.code == "00") {
                        layer_alert(response.message);
                        lstPager.refresh();
                        closePop();
...
function GetSingle(wId) {
    Serv.Post('gc/InsuranceType/GetSingle', {
        Id: wId
    }, function(response) {
