    ...
    methods: {
        initLayui: function(id) {
            ...
            layui.use(['element', 'form', 'table', 'laydate'], function() {
                ...
                _this.getBasic();
                _this.dptZTreeObj = new ZTreeRadio('dptName', 'deptTreeContent', 'deptTree', {}, 0, function(event, treeId, treeNode) {
                    if (treeNode.id < 0) {
                        layer_msg('请选择公司下面的部门');
                        return false;
                    }
                    _this.setDpt(treeNode);
                });
                _this.dptZTreeObj.reload();
        ...
        getBasic: function() {
            ...
            _this.basicArrival = window.globCache.getArrival() || [];
            _this.basicEducation = window.globCache.getEducation() || [];
            _this.basicProperties = window.globCache.getProperties() || [];
            _this.basicRelations = window.globCache.getRelations() || [];
            _this.basicRecruitment = window.globCache.getRecruitment() || [];
        ...
        //设置部门
        setDpt: function(model) {
            var _this = this;
            console.log(model);
            _this.person.dptId = model.id;
            _this.person.dptName = model.dptName;

            if (_this.person.companyId != model.companyId) {
                _this.person.companyId = model.companyId;
                _this.person.jobId = '';
                _this.getJob(model.companyId);
            }
        },
        //获取职位
        getJob: function(cId) {
